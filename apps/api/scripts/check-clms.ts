...
import {
  ...
  isCowClmWithVault,
  validateCowClms,
} from '../src/api/cowcentrated/types.js';
import { getRPCClient } from '../src/api/rpc/client.js';
import { type ApiChain, isApiChain } from '../src/utils/chain.js';
...
async function checkFile(path: string): Promise<{
  apiChain: ApiChain;
  errors: Array<{ oracleId: string; error: string }>;
}> {
