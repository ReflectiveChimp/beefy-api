...
import { MULTICHAIN_RPC } from '../src/constants.js';

import { addressBook } from 'blockchain-addressbook';
import ERC20ABI from '../src/abis/ERC20.json';
import ISolidlyPair from '../src/abis/ISolidlyPair.js';
...
async function main() {
  const farm = wrapperAddress;
  const lpToken = await fetchLp(farm);
  const lpPool = await fetchPool(lpToken.lp);
  ...
  const poolsJson = await import(poolsJsonFile);
