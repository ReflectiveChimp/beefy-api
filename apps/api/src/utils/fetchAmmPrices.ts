...
import { default as BeefyPriceMulticall } from '../abis/BeefyPriceMulticall.js';
import { fetchContract } from '../api/rpc/client.js';
import { envBoolean, envNumber } from './env.js';
import { normalizeNativeWrappedPrices } from './normalizeNativeWrappedPrices.js';
import { batchMapRetry, isContextResultFulfilled, isContextResultRejected } from './promise.js';
import { promiseTiming } from './timing.js';
  ...
  while (solving) {
    ...
    for (let i = unsolved.length - 1; i >= 0; i--) {
      ...
      for (const { knownToken, unknownToken } of trySolve) {
        const { price, weight } = calcTokenPrice(prices[knownToken.oracleId] || 0, knownToken, unknownToken);
...
function checkPoolsPrices(pools: PoolData[], prices: Record<string, number>) {
  const results = pools.map((pool) => {
    const lp0Price = prices[pool.lp0.oracleId] || 0;
    const lp1Price = prices[pool.lp1.oracleId] || 0;
...
async function fetchChainPools(chain: ChainId, pools: Pool[]): Promise<PoolData[]> {
  ...
  if (!multicallAddress) {
    throw new Error(`No multicall address found for ${chain}`);
  }
