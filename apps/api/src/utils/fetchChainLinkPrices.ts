...
import { chainLinkOracleAbi } from '../abis/ChainLinkOracle.js';
import { fetchContract } from '../api/rpc/client.js';
import { fromWei } from './big-number.js';
import { type ApiChain, toChainId } from './chain.js';
import { median } from './number.js';
import { contextAllSettled, isResultFulfilled } from './promise.js';
...
export async function fetchChainLinkPrices(): Promise<Record<string, number>> {
  ...
  for (let i = 0; i < pricesPerChain.length; ++i) {
...
async function fetchPricesForChain(chain: ApiChain, oracles: Oracle[]): Promise<Record<string, number>> {
  const results = await contextAllSettled(oracles, async (oracle): Promise<RoundData> => {
  ...
  results.forEach((result) => {
    if (!isResultFulfilled(result)) {
      console.error(
        `ChainLink: Failed to fetch price for ${result.context.oracleId} on chain ${chain}`,
        result.reason,
      );
      return;
    }
    ...
    const oracle = result.context;
    const heartbeatAgo = subSeconds(now, oracle.heartbeat + Math.max(oracle.heartbeat * 0.2, 300)); // 20% leeway or min. 5 minutes
