...
import IAlgebraPool from '../abis/IAlgebraPool.js';
import IAlgebraPoolV1 from '../abis/IAlgebraPoolV1.js';
import IAlgebraPoolV2 from '../abis/IAlgebraPoolV2.js';
import IKyberElasticPoolAbi from '../abis/IKyberElasticPool.js';
import ISlipstreamPool from '../abis/ISlipstreamPool.js';
import IUniV3PoolAbi from '../abis/IUniV3Pool.js';
...
async function getConcentratedLiquidityPrices(
  ...
  try {
    ...
    tokenPrice.forEach((v, i) => {
      ...
      const first = chainToken.firstToken;
      const second = chainToken.secondToken;
      prices[chainToken.oracleId] =
        first == chainToken.oracleId
          ? (tokenPrices[second] || prices[second] || 0) / (chainToken.decimalDelta * Math.pow(1.0001, v))
          : (tokenPrices[first] || prices[first] || 0) * (chainToken.decimalDelta * Math.pow(1.0001, v));
    });
