...
import IJarvisPriceFeed from '../abis/IJarvisPriceFeed.js';
import { fetchNoMulticallContract } from '../api/rpc/client.js';
...
const fetchJbrlPrice = async (): Promise<object> => {
  ...
  try {
    const priceFeedContract = fetchNoMulticallContract(priceFeed, IJarvisPriceFeed, POLYGON_CHAIN_ID);
    price = new BigNumber((await priceFeedContract.read.getLatestPrice([priceId])).toString()).dividedBy(
      '1e18',
    );
  } catch (e) {
