...
import { fetchContract, getRPCClient } from '../api/rpc/client.js';
...
const lpTokenPrice = async (lpToken) => {
  ...
  const tokenPairContract = fetchContract(lpToken.address, ERC20Abi, lpToken.chainId || BSC_CHAIN_ID);
  const token0Contract = fetchContract(lpToken.lp0.address, ERC20Abi, lpToken.chainId || BSC_CHAIN_ID);
  const token1Contract = fetchContract(lpToken.lp1.address, ERC20Abi, lpToken.chainId || BSC_CHAIN_ID);
...
export { lpTokenPrice, lpTokenPrices };
