...
import { isFiniteNumber } from './number.js';
...
function enhancePairs(pairs: DexScreenerPair[]): EnhancedPair[] {
  return (
    pairs
      ...
      .map((pair) => ({
        chainId: dexScreenerChainIdToChainId[pair.chainId]!, // checked in filter
        baseToken: {
          ...pair.baseToken,
          priceUsd: Number.parseFloat(pair.priceUsd!), // checked in filter
        },
        quoteToken: {
          ...pair.quoteToken,
          priceUsd: (1 / Number.parseFloat(pair.priceNative)) * Number.parseFloat(pair.priceUsd!), // checked in filter
        },
