...
import ISolidlyPair from '../abis/ISolidlyPair.js';
import { fetchContract } from '../api/rpc/client.js';
import type { ApiChain } from './chain.js';
...
async function getStablePoolPrices(
  ...
  try {
    ...
    tokenPrice.forEach((v, i) => {
      ...
      const second = chainToken.secondToken;
      const amount = new BigNumber(v).dividedBy(new BigNumber(chainToken.firstTokenDecimals)).toNumber();
      prices[chainToken.oracleId] = (tokenPrices[second] || 0) / amount;
    });
...
export async function fetchSolidlyStableTokenPrices(
  tokenPrices: Record<string, number>,
): Promise<Record<string, number>> {
  const pricesByChain: Record<string, number>[] = await Promise.all(
    Object.entries(tokens).map(async ([chainId, chainTokens]) => {
      const prices = await getStablePoolPrices(tokenPrices, chainTokens, ChainId[chainId as ApiChain]);
