...
import { MULTICHAIN_ENDPOINTS } from '../../constants.js';
import { serviceEventBus } from '../../utils/ServiceEventBus.js';
import { type ApiChain, isApiChain, toApiChain } from '../../utils/chain.js';
import { getChainNewBoosts } from '../boosts/getBoosts.js';
import { getSingleChainVaults, getVaultsByChain } from '../stats/getMultichainVaults.js';
import type { ChainTokens, TokenEntity, TokenErc20, TokenNative, TokensByChain } from './types.js';
...
async function fetchTokensForChain(chainId: ApiChain): Promise<ChainTokens> {
  ...
  abTokens.forEach((token) => {
    ...
    const existing = byAddress[addressKey];
    if (existing) {
      existing.oracleId = token.oracleId;
