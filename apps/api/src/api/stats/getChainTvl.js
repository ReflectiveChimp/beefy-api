...
const getChainTvl = async (chain) => {
  ...
  const [vaultBalances, govVaultBalances, cowVaultBalances, erc4624VaultBalances] =
    await Promise.all(vaultsCalls);
  ...
  for (const clm of cowVaults) {
    ...
    if (clmPool && clmVault) {
      ...
      const clmPoolItem = {
        [clmPool.id]: Math.max(0, clmPoolTvl - clmVaultTvl),
      };
