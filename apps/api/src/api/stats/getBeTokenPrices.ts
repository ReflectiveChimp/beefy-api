...
import { sonicStakingAbi } from '../../abis/sonic/sonicStakingAbi.js';
import { bigintRange } from '../../utils/array.js';
import { BIG_ONE, BIG_UNIT_18, BIG_ZERO } from '../../utils/big-number.js';
...
export async function getBeTokenPrices(tokenPrices: Record<string, number>): Promise<Record<string, number>> {
  ...
  for(const [to, from] of Object.entries(priceMap)) {
    const value = tokenPrices[from];
    if (value !== undefined) {
      output[to] = value;
    }
    else {
      console.warn(`Could not set ${to} price as ${from} price is missing`);
    }
  }

  const WS = tokenPrices['WS'];
  if (WS !== undefined) {
    output['beS'] = await getBeSonicRedeemablePrice(WS);
  }
  else {
    console.warn(`Could not set beS price as WS price is missing`);
  }
