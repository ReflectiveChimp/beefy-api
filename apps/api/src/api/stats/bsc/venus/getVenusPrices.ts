...
import { isFiniteBigNumber } from '../../../../utils/big-number.js';
...
export const fetchVenusPrices = async (
  ...
  const exchangeRates = await Promise.all(
    vTokens.map(async (vToken) => {
      try {
        ...
        const currentExchangeRate = new BigNumber((await contract.read.exchangeRateCurrent()).toString());
        if (isFiniteBigNumber(currentExchangeRate)) {
          return currentExchangeRate;
        } else {
          console.log(`Error fetching venus price for ${vToken.oracleId}: invalid exchangeRateCurrent`);
        }
      } catch (err) {
  ...
  return vTokens.reduce(
    (prices, vToken, i) => {
      ...
      const exchangeRate = exchangeRateWei.shiftedBy(-(18 + vToken.underlying.decimals - vToken.decimals));
