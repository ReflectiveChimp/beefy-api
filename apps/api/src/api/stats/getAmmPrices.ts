...
import { fetchAmmPrices } from '../../utils/fetchAmmPrices.js';
import {
  fetchBalancerLinearPoolPrice,
  fetchBalancerStablePoolPrice,
} from '../../utils/fetchBalancerStablePoolPrices.js';
import { fetchCoinGeckoPrices } from '../../utils/fetchCoinGeckoPrices.js';
import { fetchConcentratedLiquidityTokenPrices } from '../../utils/fetchConcentratedLiquidityTokenPrices.js';
import { fetchCurveTokenPrices } from '../../utils/fetchCurveTokenPrices.js';
import { fetchDefillamaPrices } from '../../utils/fetchDefillamaPrices.js';
import { fetchJbrlPrice } from '../../utils/fetchJbrlPrice.js';
import { fetchMooPrices } from '../../utils/fetchMooPrices.js';
import { fetchOptionTokenPrices } from '../../utils/fetchOptionTokenPrices.js';
import { fetchSolidlyStableTokenPrices } from '../../utils/fetchSolidlyStableTokenPrices.js';
import { fetchUnwrappedAavePrices } from '../../utils/fetchUnwrappedAaveTokenPrices.js';
import { fetchWrappedAavePrices } from '../../utils/fetchWrappedAaveTokenPrices.js';
import { fetchXPrices } from '../../utils/fetchXPrices.js';
import { fetchyVaultPrices } from '../../utils/fetchyVaultPrices.js';
...
import { fetchChainLinkPrices } from '../../utils/fetchChainLinkPrices.js';
import { type OraclePriceRequest, fetchDexScreenerPriceOracles } from '../../utils/fetchDexScreenerPrices.js';
import {
  debugNativeWrappedPrices,
  normalizeNativeWrappedPrices,
} from '../../utils/normalizeNativeWrappedPrices.js';
import { isFiniteNumber } from '../../utils/number.js';
import { sleep } from '../../utils/time.js';
import { promiseTiming } from '../../utils/timing.js';
import { fetchVenusPrices } from './bsc/venus/getVenusPrices.js';
import { getBeTokenPrices } from './getBeTokenPrices.js';
import { getLpBasedPrices } from './getLpBasedPrices.js';
import getNonAmmPrices from './getNonAmmPrices.js';
...
async function fetchSeedPrices() {
  ...
  for (const [geckoId, oracleIds] of Object.entries(coinGeckoCoins)) {
    for (const oracleId of oracleIds) {
      ...
      } else if (cg === undefined && dl !== undefined) {
        seedPrices[oracleId] = dl;
      } else if (dl === undefined && cg !== undefined) {
        seedPrices[oracleId] = cg;
      } else {
        console.warn(`${oracleId} not found on CoinGecko or Defillama`);
      }
    }
  }
  ...
  for (const [oracle, peggedOracle] of Object.entries(seedPeggedPrices)) {
    const peggedPrice = seedPrices[peggedOracle];
    if (peggedPrice !== undefined) {
      seedPrices[oracle] = peggedPrice;
    } else {
...
async function updateAmmPrices() {
  ...
  try {
    ...
    console.error(`> error updating amm prices (${(Date.now() - start) / 1000}s)`, errorToString(err));
  } finally {
...
export async function getLpBreakdownForOracle(oracleId: string) {
  ...
  const breakdown = breakdowns[oracleId];
  if (!breakdown) {
    throw new Error(`breakdown not found for ${oracleId}`);
  }
  return breakdown;
}
