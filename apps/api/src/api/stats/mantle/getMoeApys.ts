...
const getFarmApys = async (): Promise<BigNumber[]> => {
  ...
  for (let i = 0; i < pools.length; i++) {
    ...
    const moePerSec = moePerSecPerId[i];
    if (!balance || !moePerSec) {
      console.error(`${pool.name}: error fetching farm apy`);
      apys.push(BIG_ZERO);
      continue;
    }
    ...
    const poolRewards = moePerSec.times(1 - treasuryShare);
