...
import AuraGauge from '../../../abis/ethereum/AuraGauge.jsx';
import { DAILY_HPY, ETH_CHAIN_ID } from '../../../constants.js';
...
import { getAuraData } from './getAuraApys.js';
...
const getYearlyRewardsInUsd = async () => {
  ...
  const bbausdYearlyRewards = bbaUSDRewardRate.times(3).times(BLOCKS_PER_DAY).times(365);
...
const getTotalStakedInUsd = async () => {
  ...
  const totalStaked = new BigNumber((await tokenContract.read.balanceOf([auraBalGauge])).toString());
  const tokenPrice = await fetchPrice({ oracle: ORACLE, id: auraBAL.oracleId });

  return totalStaked.times(tokenPrice).dividedBy(DECIMALS);
};

export default getAuraBalApy;
