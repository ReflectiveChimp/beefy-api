import BigNumber, { type BigNumber as BigNumberStatic } from 'bignumber.js';
import type { ChainId } from 'blockchain-addressbook';
import jp from 'jsonpath';
import type { Abi } from 'viem';
import IComptroller from '../../../abis/IComptroller.js';
...
const getCompoundV2ApyData = async (params: CompoundV2ApyParams) => {
  ...
  const { supplyApys, borrowApys, supplyCompApys, borrowCompApys } = await getPoolsApys(params, poolsData);

  const liquidStakingApys = await getLiquidStakingApys(params.pools);

  const apys: BigNumber[] = [];
  params.pools.forEach((pool, i) => {
    const apy = getPoolLeveragedApy(pool, supplyApys[i], borrowApys[i], supplyCompApys[i], borrowCompApys[i]);
...
const getPoolLeveragedApy = (
  ...
  const { leveragedSupplyBase, leveragedBorrowBase, leveragedSupplyComp, leveragedBorrowComp } =
    getLeveragedApys(supplyApy, borrowApy, supplyCompApy, borrowCompApy, pool.borrowDepth, pool.borrowRate);
...
const getPoolsData = async (params: CompoundV2ApyParams): Promise<PoolsData> => {
  ...
  const pricePromises = params.pools.map((pool) => fetchPrice({ oracle: pool.oracle, id: pool.oracleId }));
  ...
  const cTokenDecimals: BigNumber[] = res[7].map((v) => new BigNumber(10).exponentiatedBy(v.toString()));
...
const getLeveragedApys = (
  ...
  const bigBorrowPercent = new BigNumber(borrowPercent);
  ...
  for (let i = 0; i < depth; i++) {
    leveragedSupplyBase = leveragedSupplyBase.plus(supplyBase.times(bigBorrowPercent.exponentiatedBy(i)));
    leveragedSupplyComp = leveragedSupplyComp.plus(supplyNative.times(bigBorrowPercent.exponentiatedBy(i)));

    leveragedBorrowBase = leveragedBorrowBase.plus(borrowBase.times(bigBorrowPercent.exponentiatedBy(i + 1)));
