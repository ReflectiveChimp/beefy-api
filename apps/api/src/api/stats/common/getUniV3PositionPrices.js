...
import BeefyUniswapPositionHelperAbi from '../../../abis/BeefyUniswapPositionHelper.jsx';
...
const getUniV3PositionPrices = async (params) => {
  ...
  for (let i = 0; i < params.pools.length; i++) {
    const price = await getPrice(params.pools[i], positionTokens[i], params.tokenPrices);
    prices = { ...prices, ...price };
  }
