...
import { default as ICurveGauge } from '../../../../abis/ICurveGauge.jsx';
import { default as ICurveRewardStream } from '../../../../abis/ICurveRewardStream.jsx';
import { default as ICurveRewards } from '../../../../abis/ICurveRewards.jsx';
...
export const getCurveSubgraphApys = async (pools, url) => {
  ...
  try {
    ...
    pools.forEach((pool) => {
      const apy = new BigNumber(getSubgraphDataApy(apyData, pool.pool));
...
const getSubgraphDataApy = (apyData, poolAddress) => {
  try {
    const pool = apyData.find((p) => p.address.toLowerCase() === poolAddress.toLowerCase());
    if (!pool) return 0;
    const apy = Math.max(pool.latestDailyApy, pool.latestWeeklyApy);
...
const getFactoryApy = (factoryApyData, poolAddress) => {
  try {
    const pool = factoryApyData.find((p) => p.poolAddress.toLowerCase() === poolAddress.toLowerCase());
