...
import IStableSwapAbi from '../../../../abis/IStableSwap.jsx';
import { getTradingFeeAprHop } from '../../../../utils/getTradingFeeApr.js';
import { fetchContract } from '../../../rpc/client.js';
import getApyBreakdown from '../getApyBreakdown.js';
import { getFarmApys } from '../getRewardPoolApys.js';
...
const getTvl = async (params) => {
  ...
  const [lpResults, poolResults] = await Promise.all([Promise.all(lpCalls), Promise.all(poolCalls)]);
  ...
  for (const pool of params.pools) {
    tvls.push(totalSupplys[i].times(virtualPrices[i]).times(pool.virtualDecimals).dividedBy('1e36'));
    ++i;
  }
