...
import { default as IBalancerVaultV3 } from '../../../../abis/IBalancerVaultV3.jsx';
...
const getBalancerV3Prices = async (chainId, pools, tokenPrices) => {
  ...
  for (let i = 0; i < pools.length; i++) {
    const price = await getPoolPrice(pools[i], tokenAddresses[i], balances[i], totalSupplys[i], tokenPrices);
    prices = { ...prices, ...price };
  }
