...
import DMMPool from '../../../abis/matic/DMMPool.jsx';
...
const getAprs = async (params) => {
  ...
  for (let i = 0; i < params.pools.length; i++) {
    ...
    if (endTimes[i] > Date.now() / 1000) {
      ...
      if (pool.rewards) {
        const rewardPrice = await fetchPrice({
          oracle: 'tokens',
          id: pool.rewards[0].oracleId,
        });
        const yearlyRewardsExtra = new BigNumber(rewardRates[i][pool.rewardIndex]).times(secondsPerYear);
    ...
    if (params.log) {
      console.log(pool.name, yearlyRewardsInUsd.toNumber(), totalStakedInUsd.toNumber(), apr.toNumber());
    }
