...
import IStakingDualRewards from '../../../abis/StakingDualRewards.js';
import { isSushiClient } from '../../../apollo/client.js';
import type { LpPool } from '../../../types/LpPool.js';
import { fetchPrice } from '../../../utils/fetchPrice.js';
import { getTradingFeeApr, getTradingFeeAprSushi } from '../../../utils/getTradingFeeApr.js';
...
const getFarmApys = async (params: DualRewardPoolParams) => {
  const apys = [];
  let rewardATokenPrice = await fetchPrice({
    oracle: params.oracleA,
    id: params.oracleIdA,
  });
  let rewardBTokenPrice = await fetchPrice({
    oracle: params.oracleB,
    id: params.oracleIdB,
  });
  const decimalsA = params.decimalsA;
  ...
  for (let i = 0; i < params.pools.length; i++) {
    ...
    if (pool['rewardB']) {
      const rewardB = pool['rewardB'];
      decimalsB = rewardB.decimals;
      rewardBTokenPrice = await fetchPrice({
        oracle: rewardB.oracle,
        id: rewardB.oracleId,
      });
    }
...
const getPoolsData = async (params: DualRewardPoolParams) => {
  ...
  params.pools.forEach((pool) => {
    ...
    if (pool.name == 'quick-quick-tel' || pool.name == 'quick-usdc-tel' || pool.name == 'quick-weth-tel') {
