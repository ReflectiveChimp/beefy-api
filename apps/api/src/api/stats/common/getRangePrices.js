...
import RangeAbi from '../../../abis/Range.jsx';
import { fetchContract } from '../../rpc/client.js';

export const getRangePrices = async (chainId, pools, tokenPrices) => {
  const [amountCalls, totalSupplyCalls] = pools.reduce(
    (acc, pool) => {
      const contract = fetchContract(pool.address, RangeAbi, chainId);
      acc[0].push(contract.read.getUnderlyingBalances());
...
const getTokenPrice = (tokenPrices, oracleId) => {
  ...
  if (tokenPrices.hasOwnProperty(tokenSymbol)) {
    ...
    console.error(`Range Unknown token '${tokenSymbol}'. Consider adding it to .json file`);
  }
  return tokenPrice;
};

export default getRangePrices;
