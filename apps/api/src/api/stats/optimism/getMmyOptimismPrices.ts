...
const getMmyOptimismPrices = async (tokenPrices: Record<string, number>) => {
  let prices: Record<string, BaseLpBreakdown | number> = {};
  const promises: Array<Promise<Record<string, BaseLpBreakdown | number>>> = [];
...
const getPrice = async (pool: Pool, tokenPrices: Record<string, number>) => {
  if (pool.oracle == 'lps') {
...
const getTokenPrice = (tokenPrices: Record<string, number>, oracleId: string) => {
  ...
  if (tokenPrice === undefined) {
    console.error(`[Mmy] Unknown token '${oracleId}'. Consider adding it to .json file`);
  }
  return tokenPrice ?? 1;
};
...
export default getMmyOptimismPrices;
