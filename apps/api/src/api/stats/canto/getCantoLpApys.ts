...
import CantoLendAbi from '../../../abis/canto/CantoLend.js';
import { CANTO_CHAIN_ID } from '../../../constants.js';
import { CANTO_CHAIN_ID as chainId } from '../../../constants.js';
import volatilePools from '../../../data/canto/cantoLpPools.json';
import stablePools from '../../../data/canto/cantoStableLpPools.json';
...
import { type ApyBreakdownResult, getApyBreakdown } from '../common/getApyBreakdown.js';
...
const getCantoLpApys = async (): Promise<ApyBreakdownResult> => {
  const { compRates, totalSupplys, exchangeRates }: PoolsData = await getPoolsData(pools);

  const aprPromises = pools.map((p, i) => getPoolApy(p, compRates[i], totalSupplys[i], exchangeRates[i]));
...
const getPoolApy = async (pool, compRate, totalSupply, exchangeRate) => {
  ...
  const [rewardPrice, tokenPrice] = await Promise.all([
