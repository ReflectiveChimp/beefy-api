...
import FerroFarm from '../../../abis/cronos/FerroFarm.jsx';
import { CRONOS_CHAIN_ID, CRONOS_CHAIN_ID as chainId } from '../../../constants.js';
...
const getTradingApys = async () => {
  ...
  try {
    ...
    pools.forEach((pool) => {
      const apy = new BigNumber(getApiData(apyData, pool.key));
...
const getApiData = (apyData, poolKey) => {
  try {
    const pool = apyData[poolKey];
    if (!pool) return 0;
    const apy = pool.baseApr;
