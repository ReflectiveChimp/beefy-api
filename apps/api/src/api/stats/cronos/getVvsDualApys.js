...
import Craftsman from '../../../abis/cronos/Craftsman.jsx';
import CraftsmanV2 from '../../../abis/cronos/CraftsmanV2.jsx';
import VVSRewarder from '../../../abis/cronos/VVSRewarder.jsx';
import { vvsClient } from '../../../apollo/client.js';
import { BASE_HPY, CRONOS_CHAIN_ID } from '../../../constants.js';
import pools from '../../../data/cronos/vvsDualLpPools.json';
...
const getVvsDualApys = async () => {
  ...
  for (let i = 0; i < pools.length; i++) {
    ...
    if (!tokenPerSecData[i].isNaN() && rewardEndData[i].isGreaterThan(currentTimestamp)) {
      const tokenBPerSec = tokenPerSecData[i];
...
const getPoolsData = async (pools) => {
  ...
  rewarders.forEach((rewarder) => {
    const rewarderContract = fetchContract(rewarder, VVSRewarder, CRONOS_CHAIN_ID);

    rewardEndCalls.push(rewarderContract.read.rewardEndTimestamp());
    tokenPerSecCalls.push(rewarderContract.read.rewardPerSecond());
  });

  const resRewards = await Promise.all([Promise.all(rewardEndCalls), Promise.all(tokenPerSecCalls)]);
