...
import type { TreasuryApiResult, ValidatorAsset } from './types.js';
...
type SonicValidator = Omit<Required<ValidatorAsset>, 'method'> & {
  method: 'sonic-contract';
};
...
export const fetchAPIValidatorBalance = async (apiAsset: ValidatorAsset): Promise<TreasuryApiResult> => {
  const balance: number = await fetch(apiAsset.methodPath)
    .then((res) => res.json())
