...
import { contextAllSettled } from '../../utils/promise.js';
import { validatorStructure } from './validatorStructure.js';
...
const updateValidatorPerformance = async () => {
  ...
  try {
    const settledPromises = await contextAllSettled(
      Object.entries(validatorStructure) as Array<
        [
          keyof typeof validatorStructure,
          { validatorFunctionName: () => Promise<FetchValidatorPerformanceResponse> },
        ]
      >,
      async ([_, val]) => val.validatorFunctionName(),
    );
    settledPromises.forEach((settledPromise, i) => {
      if (settledPromise.status === 'fulfilled') {
        validatorPerformance[settledPromise.context[0]] = {
