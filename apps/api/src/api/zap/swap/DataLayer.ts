...
export class DataLayer {
  protected tokenSupport: TokenSupportByChainByProviderByAddress = keysToObject(ApiChains, () => ({}));
  ...
  protected async performGarbageCollection() {
    ...
    for (const _apiChain of Object.keys(this.tokenSupport)) {
      const apiChain = _apiChain as ApiChain;
      const blockedTokens = blockedTokensByChain[apiChain];

      for (const providerId of Object.keys(this.tokenSupport[apiChain])) {
        const supportByAddress = this.tokenSupport[apiChain]![providerId as ProviderId]!;
