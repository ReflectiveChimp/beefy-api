...
import { getAmmAllPrices } from '../../stats/getAmmPrices.js';
import { getSingleChainCowVaults, getSingleChainVaults } from '../../stats/getMultichainVaults.js';
import { getTokenById, getTokenNative, getTokenWrappedNative, isTokenErc20 } from '../../tokens/tokens.js';
import type { TokenEntity, TokenErc20 } from '../../tokens/types.js';
import { blockedTokensByChain } from './blocked-tokens.js';
import { checkBuy } from './buy.js';
import type { ISwapProvider } from './providers/ISwapProvider.js';
import { type ProviderId, providers, providersById } from './providers/index.js';
import { checkSell } from './sell.js';
import {
  type BuyTestToken,
  type SellTestToken,
...
export async function fetchProviderSupportForChainTokens(
  ...
  const tokenPrices: (number | undefined)[] = tokensToCheck.map((token) =>
    token.oracleId && isFiniteNumber(beefyPrices[token.oracleId]) && (beefyPrices[token.oracleId] || 0) > 0
