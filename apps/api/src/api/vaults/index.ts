import { sendBadRequest, sendNotFound, sendSuccess, withErrorHandling } from '../../utils/koa.js';
import {
...
} from '../stats/getMultichainVaults.js';
import { getVaultFees } from './getVaultFees.js';
import { withChainId } from './helpers.js';
...
export const singleVault = withErrorHandling(async (ctx) => {
  ...
  if (vault.type !== 'standard') {
    sendBadRequest(ctx, {
      error: `Vault with id ${vaultId} is not a standard vault`,
    });
    return;
  }
...
export const singleGovVault = withErrorHandling(async (ctx) => {
  ...
  if (vault.type !== 'gov') {
    sendBadRequest(ctx, {
      error: `Vault with id ${vaultId} is not a gov vault`,
    });
    return;
  }
...
export const singleCowVault = withErrorHandling(async (ctx) => {
  ...
  if (vault.type !== 'cowcentrated') {
    sendBadRequest(ctx, {
      error: `Vault with id ${vaultId} is not a cowcentrated vault`,
    });
    return;
  }
...
export const singleHarvestableVault = withErrorHandling(async (ctx) => {
  ...
  if (!['standard', 'cowcentrated', 'erc4624'].includes(vault.type)) {
    sendBadRequest(ctx, {
      error: `Vault with id ${vaultId} is not harvestable`,
    });
    return;
  }
