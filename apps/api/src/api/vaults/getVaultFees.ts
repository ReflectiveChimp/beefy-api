...
import FeeABI from '../../abis/FeeABI.js';
import { isDefined } from '../../utils/array.js';
import { getKey, setKey } from '../../utils/cache/index.js';
import { ApiChains } from '../../utils/chain.js';
import { errorToString } from '../../utils/error.js';
import { fetchContract } from '../rpc/client.js';
import { getHarvestableVaultsByChain } from '../stats/getMultichainVaults.js';
import type { HarvestableVault } from './types.js';
...
const updateVaultFees = async () => {
  ...
  const promises = ApiChains.map((chain) => {
    ...
    if (chainVaults.length > 0) {
      ...
      const feeBatch = feeBatches[ChainId.ethereum];
      if (!feeBatch) {
        throw new Error('No fee batch found on ethereum');
      }
      return getChainFees(chainVaults, ChainId[chain], feeBatch);
    }
  }).filter(isDefined);
...
const getChainFees = async (vaults: HarvestableVault[], chainId: number, feeBatch: FeeBatchDetail) => {
  try {
    const calls = vaults.map(async (vault) => {
      ...
      if (!strategy) {
        throw new Error(`${vault.id} missing strategy address`);
      }

      const contract = fetchContract(strategy, FeeABI, chainId);
    ...
    results.forEach((res, index) => {
      const vault = vaults[index]!; // results 1:1 vaults
...
const mapStrategyCallsToFeeBreakdown = (
  ...
  const performanceFee = performanceFeesFromCalls(contractCalls, feeBatch);

  const depositFee = depositFeeFromCalls(contractCalls);
...
const withdrawalFeeFromCalls = (contractCalls: StrategyCallResponse): number => {
  ...
  const { paused, withdraw, withdraw2, withdrawMax, withdrawMax2 } = contractCalls;
  if (paused) {
    return 0;
  }

  const withdrawFee = withdraw ?? withdraw2;
  const maxWithdrawFee = withdrawMax ?? withdrawMax2;
...
const legacyFeeMappings = (
  ...
  const callFee = contractCalls.call ?? contractCalls.call2 ?? contractCalls.call3 ?? contractCalls.call4;
  const strategistFee = contractCalls.strategist ?? contractCalls.strategist2;
  const maxFee = contractCalls.maxFee ?? contractCalls.maxFee2 ?? contractCalls.maxFee3;
  const beefyFee = contractCalls.beefy;
  const fee = contractCalls.fee;
  const treasury = contractCalls.treasury;
  const rewards = contractCalls.rewards ?? contractCalls.rewards2;

  if (
    callFee !== undefined &&
    strategistFee !== undefined &&
    beefyFee !== undefined &&
    callFee + strategistFee + beefyFee === maxFee
  ) {
    performanceFee = {
      total,
    ...
    rewards !== undefined &&
    treasury !== undefined &&
    callFee + strategistFee + rewards + treasury === maxFee
  ) {
    performanceFee = {
      total,
      call: (total * callFee) / maxFee,
  ...
  } else if (fee !== undefined && callFee !== undefined && fee + callFee === maxFee) {
    total = 0.05;
    performanceFee = {
      total: total,
      call: (total * callFee) / maxFee,
    ...
    beefyFee !== undefined &&
    maxFee !== undefined &&
    !isNaN(strategistFee + callFee + beefyFee)
  ) {
    performanceFee = {
      total,
      call: (total * callFee) / maxFee,
    ...
    treasury !== undefined &&
    maxFee !== undefined &&
    !isNaN(strategistFee + callFee + treasury)
  ) {
    performanceFee = {
      total,
      call: (total * callFee) / maxFee,
    ...
    treasury !== undefined &&
    rewards !== undefined &&
    callFee + treasury + rewards === maxFee
  ) {
    performanceFee = {
      total,
      call: (total * callFee) / maxFee,
  ...
  } else if (callFee !== undefined && beefyFee !== undefined && callFee + beefyFee === maxFee) {
  ...
  } else if (callFee !== undefined && fee !== undefined && callFee + fee === maxFee) {
    performanceFee = {
      total,
      call: (total * callFee) / maxFee,
      strategist: 0,
...
const performanceFromGetFees = (
  ...
  const total = fees.total.div(1e9).toNumber() / 1e9;
  const beefy = fees.beefy.div(1e9).toNumber() / 1e9;
  const call = fees.call.div(1e9).toNumber() / 1e9;
  const strategist = fees.strategist.div(1e9).toNumber() / 1e9;
...
const performanceForMaxi = (contractCalls: StrategyCallResponse): PerformanceFee | undefined => {
  ...
  const strategyAddress = contractCalls.strategy.toLowerCase();
  ...
  if (
    ...
    '0x87056F5E8Dce0fD71605E6E291C6a3B53cbc3818'.toLowerCase() === strategyAddress &&
    callFee !== undefined &&
    rewards !== undefined &&
    maxFee !== undefined
  ) {
  ...
  } else if (callFee !== undefined && maxCallFee !== undefined) {
    performanceFee = {
