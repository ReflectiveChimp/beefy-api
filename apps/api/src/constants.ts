...
import { getChainRpcs } from './api/rpc/rpcs.js';
...
const RPCS_BY_CHAIN = mapValues<ApiChainToRpcs, string[]>(DEFAULT_RPCS, (hereDefaultRpcs, chain) => {
  const customRpcs = getRpcsFromEnv(chain as ApiChain);
  const otherDefaultRpcs = getChainRpcs(toChainId(chain as ApiChain));
  // FIXME we should only have one list of rpcs
  return uniq([...customRpcs, ...hereDefaultRpcs, ...otherDefaultRpcs]);
});
...
const MULTICHAIN_RPC: Record<ChainId, string> = Object.fromEntries(
  Object.entries(RPCS_BY_CHAIN).map(([key, value]: [string, string[]]) => [
    ChainId[key as ApiChain],
    value[0]!,
  ]),
